document.addEventListener('DOMContentLoaded', function() {
    ...
    const API_URL = 'http://192.168.2.229:5000/api/summarize'; // Ganti dengan URL API yang sesuai
    ...
    summarizeButton.addEventListener('click', async function() {
        try
        {
            ...
            if(!result.success)
                throw new Error(result.message || 'Gagal mendapatkan ringkasan dari server.');

            // Asumsikan server mengembalikan ringkasan dalam format JSON
            const summary = result.data.summary || 'Tidak ada ringkasan yang tersedia.';
