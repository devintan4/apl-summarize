...
const PdfSummary = ({ summaries, onNewUpload, onNewUrl }: PdfSummaryProps) => {
  ...
  const handleAddFile = () => {
    ...
    input.onchange = async (e: Event) => {
      ...
      if (!files || files.length === 0) {
        return; // no files, keluar
      }

      // sekarang files: FileList pasti ada dan length>0
      const file = files[0];
  ...
  const handleAddUrl = async (e: React.FormEvent) => {
    e.preventDefault();
    const isPdfLink = /\.pdf($|\?)/i.test(newPdfUrl);
    const isDriveLink = /drive\.google\.com\/file\/d\/[A-Za-z0-9_-]+/i.test(
      newPdfUrl
    );
