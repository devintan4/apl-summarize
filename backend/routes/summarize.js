...
router.post("/", upload.single("file"), (req, res) => {
  ...
  const pythonExec = `C:/Users/vnine/Miniconda3/envs/pdf-summarizer/python.exe
PORT=3000`;
  ...
  exec(
    `${pythonExec} "${pythonScript}" "${filePath}" -o "${outputPath}"`,
    (err, stdout, stderr) => {
      if (err) {
      ...
      fs.readFile(outputPath, "utf-8", (readErr, data) => {
        if (readErr) {
          return res
            .status(500)
            .json({ error: "Could not read summary output" });
        }
